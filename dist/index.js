...
let pieceMatrixStack = new Array(5)
  .fill(0)
  .map((x) => valToBlock(randomizePiece(pieces)));
...
function valToBlock(piece) {
  return piece.matrix.map((row) =>
    row.map((val) => new Block(val, piece.color))
  );
}
...
function drawMatrix({ matrix, offset = { x: 0, y: 0 }, ctx }) {
  matrix.map((row, y) => {
    row.map((block, x) =>
      drawBlock({ block, x: x + offset.x, y: y + offset.y, ctx })
    );
  });
}
...
function handleRight() {
  ...
  player.pos.x++;
  if (
    player.pos.x + mWidth > BOARD_WIDTH ||
    collision(arena, player.pos, player.matrix)
  ) {
    player.pos.x--;
  }
}
